...
export interface IArtisanProfile extends Document {
  ...
  cancellationRate: number;    // % jobs cancelled by artisan (0-100)
  disputeRate: number;         // % jobs with disputes (0-100)
  onTimeRate: number;          // % jobs completed by deadline (0-100)
  responseTime: number;        // Avg minutes to accept/reject

  // Trust scoring
  trustScore: number;          // Weighted composite (0-100)
...
const artisanProfileSchema = new Schema<IArtisanProfile>(
  {
    ...
    // Stats tracking (for calculating rates)
