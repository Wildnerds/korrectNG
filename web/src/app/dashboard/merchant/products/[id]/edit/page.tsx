...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      const res = await apiFetch<Product>(`/products/${productId}`, { token });
      if (res.data) {
        ...
        setFormData({
          ...
          compatibleTrades: product.compatibleTrades || [],
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const totalImages = formData.images.length + formData.newImages.length + files.length;
    if (totalImages > 5) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadedImages: ProductImage[] = [...formData.images];
    ...
    <div className="min-h-screen bg-brand-light-gray py-8">
      <div className="max-w-3xl mx-auto px-4">
        <div className="mb-6">
          ...
          <p className="text-brand-gray">Changes to name, description, or price will require re-approval.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-5 gap-4">
              ...
              {/* Add More Button */}
              {formData.images.length + newImagePreviews.length < 5 && (
