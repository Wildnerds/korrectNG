...
export default function CustomerContractsPage() {
  ...
  const pendingSignature = contracts.filter(
    c => c.status === 'pending_signatures' && !c.customerSignature
  );
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-brand-gray">View and manage your service contracts</p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  Review and sign to proceed with the service.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {['all', 'pending_signatures', 'signed', 'active', 'completed', 'disputed'].map(status => (
            <button
              key={status}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {contracts.map(contract => {
              ...
              return (
                <Link key={contract._id} href={`/dashboard/customer/contracts/${contract._id}`}>
                  <div className={`bg-white rounded-xl p-5 hover:shadow-lg transition-shadow ${
                    needsSignature ? 'border-2 border-yellow-300' : ''
                  }`}>
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-brand-gray text-sm mb-2">
                          with {(contract.artisan as any)?.firstName} {(contract.artisan as any)?.lastName}
                          {(contract.artisanProfile as any)?.businessName &&
                            ` (${(contract.artisanProfile as any)?.businessName})`
                          }
                        </p>
